...
class datetimeUtils {
     ...
     * @return {number} 星期
     */
    static calcLocalWeekdayByDate(paramDate) {
        return this.calcLocalWeekdayByUTC(paramDate.getTime());
    }

    /**
     * 根据utc时间 计算本地星期 时间单位毫秒
     * @param {number} paramUTC 指定的时间戳
     * @return {number} 星期
     */
    static calcLocalWeekdayByUTC(paramUTC) {
        // 这个天数是从1970算起的天数
        let days = this.CalcLocalDaysByUTC(paramUTC);
        return (days + 4) % DatetimeConstant.WEEKDAY;
    }
    /**
     * 计算当前时间 本地星期天0点 utc时间
     * @param  {number} paramUTC 指定的UTC时间 单位毫秒
     * @return {number} 星期天0点 utc时间 单位毫秒
     */
    static calcLocalSundayZeroTime(paramUTC) {
        let todayZero = datetimeUtils.getTodayZeroTime(paramUTC);
        let weekday   = this.calcLocalWeekdayByUTC(paramUTC);
        return todayZero - weekday * DatetimeConstant.MILLIS_PRE_DAY;
    }
}
